...
def draw():
    ...
    if game_over:
        display_message("GAME OVER!", "Want to try again? Press the space bar.")
    elif game_complete:
        display_message("YOU WON!", "Want to try again? Press the space bar.")
    else:
        for star in stars:
            star.draw()
...
def shuffle():
    ...
    if stars:
        ...
        for index, star in enumerate(stars):
            ...
            new_y = y_values[index]
            animation = animate(star, duration = 0.5, x = new_x, y = new_y)
            animations.append(animation)
